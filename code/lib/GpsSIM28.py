...
class GPSSIM28:
    ...
    def __init__(self,config, logger):
        ...
        # Having a lock is necessary, because it is possible to have two gps threads running at the same time -- not any longer, (validate)
    ...
    def gps_init(self):
        ...
        self.serial = UART(0, baudrate=9600, pins=('P22', 'P21'))  # Tx, Rx

        self.chrono = Timer.Chrono()
        self.chrono.start()

        self.indicator_led = Timer.Alarm(blink_led, s=1.6, arg=(0x000055, 0.4, False), periodic=True)
    ...
    def gps_deinit(self):
        ...
        self.logger.info("Terminal re-enabled")
        self.indicator_led.cancel()
    ...
    def get_time(self,rtc):
        ...
        with self.gps_lock:
            ...
            #serial, chrono, indicator_led = 
            self.gps_init()  # initialise serial and timer
            com_counter = int(self.chrono.read())  # counter for checking whether gps is connected
            ...
            while True:
                ...
                data_in = (str(self.serial.readline()))[1:]
                ...
                if data_in[1:4] != "$GP":
                    self.logger.error("GPS sleep")
                    time.sleep(1)
                else:
                    for char in data_in:
                        self.logger.info("GPS update")
                        sentence = self.gps.update(char)
                        self.logger.info("GPS sentence" + str(sentence))
                        if sentence == "GPRMC":
                            com_counter = int(self.chrono.read())
                            if self.gps.valid:

                                # Set current time on pycom - convert seconds (timestamp[2]) from float to int
                                datetime = (int('20' + str(self.gps.date[2])), self.gps.date[1], self.gps.date[0], self.gps.timestamp[0],
                                            self.gps.timestamp[1], int(self.gps.timestamp[2]), 0, 0)
                                rtc.init(datetime)

                                # Set current time on RTC module if connected - convert seconds (timestamp[2]) from float to int
                                h_day, h_mnth, h_yr = int(str(self.gps.date[0]), 16), int(str(self.gps.date[1]), 16), int(str(self.gps.date[2]),
                                                                                                                16)
                                h_hr, h_min, h_sec = int(str(self.gps.timestamp[0]), 16), int(str(self.gps.timestamp[1]), 16), int(
                                    str(int(self.gps.timestamp[2])), 16)
                                try:
                ...
                self.logger.debug(str(self.chrono.read()))
                    ...
                    if data_in[1:4] != "$GP":
                        ...
                        for char in data_in:
                            ...
                            if sentence == "GPGGA":
                                ...
                                elapsed = self.chrono.read() / timeout
                                ...
                                if 0 < self.gps.hdop <= hdop_aim[index] and self.gps.satellites_in_use >= 3:

                                    latitude = self.gps.latitude[0] + self.gps.latitude[1]/60
                                    if self.gps.latitude[2] == 'S':
                                        latitude = -latitude

                                    longitude = self.gps.longitude[0] + self.gps.longitude[1]/60
