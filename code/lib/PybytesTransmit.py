...
import time
from ubinascii import unhexlify, a2b_base64
...
class PyBytesSender:
    def __init__(self,msgBuffer, config,  debugLogger):
        ...
        self.pybytes.update_config('pybytes_autostart', False, permanent=True, silent=False, reconnect=False)
        self.pybytes.start(autoconnect=True)  #same as connect method
        while not self.pybytes.isconnected():
            time.sleep(2)
            print(".")
            self.pybytes.start(autoconnect=True)

        self.debugLogger.info("Pybytes sender setup...")
        self.pybytes.print_config()
        #pybytes.send_info_message()
        self.pybytes.send_ping_message()
        #pdb.set_trace()
        ...
        while True:
            try:
                ...
                #get a data packet
                    ...
                    # data  = buffer_lst[2:]
                    # print("here")
                    # #msgBytes = a2b_base64(data) #debug remove,
                    # #print(msgBytes)
                    # buffer_lst = self.msgBuffer.pop() #check size?
                    # msgStr =  struct.unpack(formatStr,data)
                    # print(msgStr)

                    #print(buffer_lst)
                    # get structure and port from format
                    #timeStr = buffer_lst[1]
                    #port = buffer_lst[0]  # format is first item in the list
                    # DROP msgVersion = buffer_lst[1]
                    #lst_message = buffer_lst[1:]  #  chop year, month and format
                    ...
                    # # pack payload
                    self.debugLogger.debug("Sending over LoRa: " + str(buffer_lst))
                    ...
                    self.pybytes.send_signal(buffer_lst[0], buffer_lst[1:])
