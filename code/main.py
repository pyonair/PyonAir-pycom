...
# initialise_time # TODO: clunky refactor
...
no_time, update_time_later = init.initialise_time(
    rtc, False)  # Dont use GPS , yet - just read RTC
...
try:
    ...
    # all buffer slots are a fixed size ! so waste space, but dont make this smaller that a max message!
    cell_size_bytes = 100
    # s.processing_path, s.lora_file_name, 31 * self.message_limit, 100)
    msgBuffer = RingBuffer(RING_BUFFER_DIR, RING_BUFFER_FILE,
                           message_limit_count, cell_size_bytes,  config, status_logger)
    ...
    if config.get_config(s.PM1) == "OFF" and config.get_config(s.PM2) == "OFF":
        # TODO: Somehow make this clear that it disables BOTH???
    ...
    if sensors[s.PM1]:
        init.initialise_pm_sensor(sensor_name=s.PM1, pins=(
            'P3', 'P17'), serial_id=1, msgBuffer=msgBuffer)
    if sensors[s.PM2]:
        init.initialise_pm_sensor(sensor_name=s.PM2, pins=(
            'P11', 'P18'), serial_id=2, msgBuffer=msgBuffer)
    ...
    if False:  # gps_on and update_time_later: #TODO: gps bugs, but rememebr shared serial so check logger file!
        ...
        _thread.start_new_thread(
            GpsSIM28.SetRTCtime, (rtc, config,  status_logger))

    status_logger.info("Initialization finished")
    temp = (temperature() - 32) / 1.8
    status_logger.info(
        "Memory:  " + str(pycom.get_free_heap()) + " Temp: " + str(temp))
